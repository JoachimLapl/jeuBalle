var NinjaAllowed = !0;
game.index = () => {
    Propeller.All.forEach(propeller => {
        var plus = propeller.f({ x: hero.x, y: hero.y }, hero.r);
        // console.log(plus)
        hero.speed.x += plus.x;
        hero.speed.y += plus.y;
    })
    CircleBlock.All.forEach(circle => {
        var plus = circle.f(hero);
        // console.log(plus)
        hero.speed.x += plus.x;
        hero.speed.y += plus.y;
    })
    ...
    // hero.speed.y < 1 && (hero.speed.y **= 2);
    // hero.speed.x < 1 && (hero.speed.x **= 2);
    // if (collides.down) console.log(onBlock)
    // console.log(hero.speed.y)
    if (!onBlock) hero.speed.y += game.gravity
    ...
    if (hero.y > rect.height + 1e3) { hero.props.x = hero.props.y = 0; hero.lives.loose(25) }
}